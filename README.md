...
The repository carries Arena Hero API v0.1 and gameplay rules v0.9 locally, so
...
- the current v0.9 rules for eight-direction Ranger fire, self-destruct,
  Worker cargo-drop, resource-node quota,
  strict `max(10, population × 5)` Core resource capacity, refill, visibility,
  contention, migration, Core resource capture, and same-Tick respawn contract;
